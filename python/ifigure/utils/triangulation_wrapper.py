...
def mask_skew(x, y, tri, th= 5, mask = None, op = np.logical_or, inv = False):
    ddd = np.vstack((np.sqrt((x[tri][:,0]- x[tri][:,1])**2+(y[tri][:,0]-y[tri][:,1])**2), np.sqrt((x[tri][:,0]- x[tri][:,2])**2+(y[tri][:,0]-y[tri][:,2])**2), np.sqrt((x[tri][:,1]- x[tri][:,2])**2+(y[tri][:,1]-y[tri][:,2])**2)))
...
def get_area(x, y, tri):
    ddd = (x[tri][:,0]-x[tri][:,1])*(y[tri][:,0]-y[tri][:,2]) - (x[tri][:,0]-x[tri][:,2])*(y[tri][:,0]-y[tri][:,1])
    return np.abs(ddd)
