...
def handle_keyevent(evt):
    ...
    if (evt.GetKeyCode() == wx.WXK_RIGHT and
         ...
         for pos in p.b_pos + p.r_pos:
             if pos[0] == p.cur_pos[0] and pos[1] == p.cur_pos[1]: return
         obj1, obj2 = draw_octagon(p.cur_pos, color=p.cur_color, 
         ...
         if p._last_obj is not None:
              ...
              if p._last_move in p.r_pos: p.r_pos.remove(p._last_move)
              if p._last_move in p.b_pos: p.b_pos.remove(p._last_move)
              if p.cur_color == 'red':
                  p.cur_color = 'black'
              else:
                  p.cur_color = 'red'
    ...
    if p.cur_pos[0] % 2 == 0:
       if p.cur_color == 'black': p.cur_dir = 'h'
       else: p.cur_dir = 'v'
    else:
       if p.cur_color == 'black': p.cur_dir = 'v'
       else: p.cur_dir = 'h'
    draw_cursor()
...
def r_check_finish(st, pos, et, rt):
    ...
    if st in pos:
       rt.append(st)
       if st in et: return True
       pos.remove(st)
       if st[1] % 2 == 1:
           if r_check_finish((st[0]+2, st[1]), pos[:], et, rt): v = True
           if r_check_finish((st[0]-2, st[1]), pos[:], et, rt): v = True
       else:
           if r_check_finish((st[0],   st[1]+2), pos[:], et, rt): v = True
           if r_check_finish((st[0],   st[1]-2), pos[:], et, rt): v = True
       if r_check_finish((st[0]+1,   st[1]+1), pos[:], et, rt): v = True
       if r_check_finish((st[0]-1,   st[1]+1), pos[:], et, rt): v = True
       if r_check_finish((st[0]+1,   st[1]-1), pos[:], et, rt): v = True
       if r_check_finish((st[0]-1,   st[1]-1), pos[:], et, rt): v = True
    return v
...
def b_check_finish(st, pos, et, rt):
    if st in pos:
       ...
       pos.remove(st)
       if st[1] % 2 == 0:
           if b_check_finish((st[0]+2, st[1]), pos[:], et, rt): return True
           if b_check_finish((st[0]-2, st[1]), pos[:], et, rt): return True
       else:
           if b_check_finish((st[0],   st[1]-2), pos[:], et, rt): return True
           if b_check_finish((st[0],   st[1]+2), pos[:], et, rt): return True
       if b_check_finish((st[0]+1,   st[1]+1), pos[:], et, rt): return True
       if b_check_finish((st[0]+1,   st[1]-1), pos[:], et, rt): return True
       if b_check_finish((st[0]-1,   st[1]+1), pos[:], et, rt): return True
       if b_check_finish((st[0]-1,   st[1]-1), pos[:], et, rt): return True
...
def highlight_route(rt):
    ...
    for x in range(20):
        time.sleep(0.1)
        if x % 2 == 0:
            for o in objs: o.set_alpha(0.8)
            viewer.draw_all()
        else:
            for o in objs: o.set_alpha(0.5)
            viewer.draw_all()
