...
for map in maps:
  ...
  bbox=Bbox.from_extents(ed[0,0]/dpi,ed[0,1]/dpi,ed[1,0]/dpi,ed[1,1]/dpi)
...
ed=ax.transAxes.transform([(0,0), (1,1)])
bbox=Bbox.from_extents((ed[0,0]+1)/dpi,(ed[0,1]+1)/dpi,ed[1,0]/dpi,(ed[1,1]-2)/dpi)
...
for name, style in arrowstyle_list():
  ax.cla()
  ax.tick_params(length=0)
  ax.annotate('', [1, 0.5], xytext=[0, 0.5], arrowprops=dict(arrowstyle=style))
  filename='arrow_'+encode(name)+'.png'
  ed=ax.transAxes.transform([(0,0), (1,1)])
  bbox=Bbox.from_extents(ed[0,0]/dpi,ed[0,1]/dpi,ed[1,0]/dpi,ed[1,1]/dpi)
