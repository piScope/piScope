...
#        print 'entering fig_grp:: generate_artist'
    ...
    def del_artist(self, artist=None, delall=False):
        ...
        if len(artistlist) != 0:
           ...
           for a in artistlist:
              if a in c.patches: c.patches.remove(a)
...
#        otherwise check children..
    def picker_a(self, a, evt):
        if self._artist_extent[0] is None: return False, {}
        ...
        children =reversed(
           sorted(
           [(child.getp('zorder'), child) for name, child in self.get_children()]
           ))
...
### group/ungroup (group is defined in FigObj)
    ...
    def get_axis_param_idx(self):            
        for name, child in self.get_children():
            ...
            if idx[0] is not None: return idx
        return [None]*4
