...
class FigPlotC(FigPlot, FigControl):
    ...
    def onEditPoint(self, evt):
        ...
        l = []
        names = ['x', 'y']
        if self.getp('xerr') is not None: names.append('xerr')
        if self.getp('yerr') is not None: names.append('yerr')
        for name in names:
        ...
        if yerr is not None: yerr[self._figc_hit] = v[names.index('yerr')]
        xerr = _copy(self.getp('xerr'))
        if xerr is not None: xerr[self._figc_hit] = v[names.index('xerr')]
    ...
    def onAddPoint(self, evt):
        ...
        for k in range(x.size-1):
           ...
           if (m1 < 0 or m1 > 1): continue
           m2 = ((x1d-xed)*(y1d-y0d) - (y1d-yed)*(x1d-x0d))/d
           if (m2 > 5 or m2 < -5): continue
           break
        else: 
           return 1
    ...
    def drag(self, a, evt):
        ...
        if evt.xdata is None: return 0
        if evt.ydata is None: return 0
