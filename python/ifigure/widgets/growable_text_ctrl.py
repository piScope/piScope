...
#        kargs['style']=wx.TE_PROCESS_ENTER|wx.TE_MULTILINE
    ...
    def setText(self, txt):
        ...
        ss = np.array([self.GetFullTextExtent(self.GetLineText(x)) for x in range(num)])

        self.SetSize((np.max(ss[:,0])+25, np.max([np.max(ss[:,1]), s[1]])*num+10))
        ...
        if x1p < x1:
            dx = x1 - x1p + 1
            if x0 > dx: self.SetPosition((x0 - dx, y0))
