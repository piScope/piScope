...
class layout_editor_popup(wx.Menu):
    def __init__(self, parent):
       super(layout_editor_popup, self).__init__(style = wx.WS_EX_PROCESS_UI_UPDATES)
       self.parent = parent
       self.mmi = []
...
#           self.parent.replace_area(area, new_areas)
    ...
    def onCommBtm(self, e):
        self.parent.layout_editor.apply_bottom_only(self.parent.layout_editor.area_hit)
    def onCommLft(self, e):
        self.parent.layout_editor.apply_left_only(self.parent.layout_editor.area_hit)
    ...
    def onResetCommon(self, evt):
       ...
       ### finish up
       send_area_request(self.parent, request = request, ac = ac, name='reset common axis')
    def update_ui(self):
        if self.parent.axes_selection() is None: return False
        if self.parent.axes_selection().figobj is None: return False
...
#       self. _load_canvas_value()
   ...
   def exit_layout_mode(self):
      if self._mid is not None: self.mpl_canvas.mpl_disconnect(self._mid)
      self._mid = None
      self.remove_artists()

   def remove_artists(self, drag = True, layout=True):
       if layout:
           for l in self.fig_hl: self.figure.lines.remove(l)
           self.fig_hl = []
           for l in self.fig_txt: self.figure.texts.remove(l)
           self.fig_txt = []
           for l in self.fig_hlp: self.figure.patches.remove(l)
           self.fig_hlp= []
       if drag:
           for l in self.fig_hl2: self.figure.lines.remove(l)
...
#       w = self.figure.get_figwidth()*self.figure.get_dpi() 
       ...
       for i, area in enumerate(self.area):
           ...
           if hit != -1: self.edge_hit.append([i, hit, False])
       for i, area in enumerate(self.area):
           hit2 = self.check_edge_hit2(area, event.x, event.y, 
                                 w, h)
           if hit2 != -1: self.edge_hit2.append([i, hit2, False])
       if len(self.edge_hit) != 0:
          ...
          if self.edge_hit3[0] != -1:
             if self._mid is None:
                 ...
                 self._mid=self.mpl_canvas.mpl_connect('motion_notify_event', self.motion)
             self.add_edge_hl()
             return 
          else:
             self.edge_hit = []
...
#              scr_size=self.mpl_canvas.GetClientSize()
              if not self._popup.update_ui(): return
...
#             wx.CallAfter(self.SetEditorValue)
         ...
         for f_axes in f_page.walk_axes():
             ...
             if area == self.area_hit: 
                ...
                break
         if ax is not None:
             ifigure.events.SendSelectionEvent(f_axes, self.canvas.GetTopLevelParent(), [])
   ...
   def motion(self, event):
       ...
       if (self.edge_hl is not None and
           ...
           request = self.process_edge_drag(event)
           self.area = area_bk
       if request is not None: self.draw_request(request)
...
#           self.edge_hl.remove()
   ...
   def process_release_outside(self, event):
       ...
       if event.x < 0: 
           ...
           earea = [(k[0], self.area[k[0]]) for k in self.edge_hit2 if k[1]==2]
           for idx, x in earea:
               x[2] = 1.-x[0]
               request.append(('m', idx, x))
           ...
           earea = [(k[0], self.area[k[0]]) for k in self.edge_hit2 if k[1]==1]
           for idx, x in earea:
               x[3] = x[3]+x[1]
               x[1] = 0
               request.append(('m', idx, x))
           ...
           earea = [(k[0], self.area[k[0]]) for k in self.edge_hit2 if k[1]==3]
           for idx, x in earea:
               x[3] = 1 - x[1]
               request.append(('m', idx, x))
       ...
       if len(request) > 0: return request
...
#       event = self._fix_event_data(event)
       ...
       if event.xdata > 1 or event.xdata < 0: event.xdata = None
       if event.ydata > 1 or event.ydata < 0: event.ydata = None
       ...
       if self.edge_hit3[4] == 1:  # boundary drag mode..
          for hit in ohit2:
             ...
             if hit[1] == 1 :
                ...
                break
             if hit[1] == 2 :
                newa = [event.xdata, self.edge_hit3[1], abs(1-event.xdata), d]
                break
             if hit[1] == 3 :
                newa = [self.edge_hit3[1], event.ydata, d, abs(1-event.ydata)]
                break
...
#           self.area = area_bk
   ...
   def find_nexeones(self, k, mode, ans = None):
       ...
       def check_dir(a1, a2, mode):
           ...
           if mode == 0: return a1[0] < a2[0]
           elif mode == 1: return a1[1] < a2[1]
           elif mode == 2: return a1[0] > a2[0]
           elif mode == 3: return a1[1] > a2[1]
       def op_mode(mode):
           if mode == 0: return 2
           elif mode == 1: return 3
           elif mode == 2: return 0
   ...
   def select_connected_edge(self, single = False):
       ...
       if single: self.edge_hit2 = [tmp[:] for tmp in self.edge_hit]
...
#            a = self.apply_pm_2_area(a)            
   ...
   def apply_left_only(self, area_hit):
       ...
       new_len = self._calc_axis_edge_only_newlen_bottomleft(f_axes_arr, 0, 1, 2)
   ...
   def apply_bottom_only(self, area_hit):
       ...
       new_len = self._calc_axis_edge_only_newlen_bottomleft(f_axes_arr, 3, 2, 3)
       ### apply new length
       request = []
...
#       self.canvas.draw()
   ...
   def _check_attached(self,a1,a2,h=True):
       ...
       if (h and (check(y1, y10) and check(y2, y20)) and
          (check(x1, x20) or check(x2,x10))): return True
...
#       self.draw()
   ...
   def SetCanvasValue(self, axes=None, request=None, ac = None, name='area'):
       ...
       if request is None:
           ...
           send_area_request(ifig_canvas, axes=axes, request=request, ac = ac, name=name)
#       ifig_canvas.draw()
