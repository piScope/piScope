...
#          )
    ...
    for z,a in alist:
       flag = True
       figobj=a.figobj
       if isinstance(figobj, FigAxes):continue
       if hasattr(a, '_is_frameart'):
           if ifig_canvas._frameart_mode:
             if not a._is_frameart: continue               
           else:
             if a._is_frameart: continue
...
def CheckLineHit(x, y, x0, y0, trans=None, itrans=None):
    ...
    try:
       ...
       ic = None
       if x.shape[0] > 100: return False, 0

    if ic is not None: return True, ic        
