...
#                  if figobj is not None: figobj.highlight_artist(False)
                ...
                if double_click:
                    ...
                    if len(self.selection):
                        if (any([s().figobj.isCompound() for s in self.selection if s(
                        ) is not None]) and event.guiEvent.ShiftDown()):
                            #
                            pass
                        else:
                            self.unselect_all()
...
#                  self.draw_later()
                    ...
                    if self.draghandler in self.draghandlers[1:11]:
                        ...
                        self.draw_later(refresh_hl=False)  # no draw until next idl e
                    else:
