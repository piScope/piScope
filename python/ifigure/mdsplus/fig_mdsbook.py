...
def convert2figmdsbook(book):
    ...
    if not hasattr(book, '_parallel_page'): book._parallel_page = True
    if not hasattr(book, '_parallel_shot'): book._parallel_shot = True
    if not hasattr(book, '_parallel_sec'): book._parallel_sec = True
    if not hasattr(book, '_use_global_tdi'): book._use_global_tdi = False
    if not hasattr(book, '_worker'): book._worker = ''
