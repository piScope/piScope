...
#        print numchunks, chunksize
        x3 = numpy.vstack((numpy.nanmin(xchunks,axis=1), numpy.nanmax(xchunks,axis=1))).transpose().reshape(chunksize*2)
#        print x3.shape
        ### append left over..
        if chunksize*numchunks < x.size:
           x2 = numpy.hstack((x3, numpy.nanmin(x[chunksize*numchunks:]), numpy.nanmax(x[chunksize*numchunks:])))
           return x2
        else:
           return x3
