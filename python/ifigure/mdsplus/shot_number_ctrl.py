...
class ShotNumberCtrl(wx.stc.StyledTextCtrl):
    def __init__(self, *args, **kargs):
        ...
        for x in range(3): self.SetMarginWidth(x, 0) 
    ...
    def set_style_bits(self):
        ...
        if txt.startswith('='):
            ...
            sarr = [[chr(k % self._len_order)*len(xx)  for k, xx in enumerate(x)]  for x in arr]
    ...
    def SetValue(self, value):
        if self._single_mode: 
            wx.stc.StyledTextCtrl.SetValue(self, (value.split(','))[0].split(';')[0])
        else:
