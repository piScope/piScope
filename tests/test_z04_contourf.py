'''
Focused tests for the interactive contourf command.
'''
...
PLOT_COMMAND = 'contourf'
ENABLE_PNG_VISUAL = True
PNG_BASELINE_SUBDIR = 'contourf'
PNG_MAE_TOL = 0.02
PNG_P99_TOL = 0.12
...
def _contourf_cases():
    X, Y, Z = _grid_data()
    return [
        ('xy_default', lambda viewer: viewer.contourf(X, Y, Z)),
        ('xy_with_levels', lambda viewer: viewer.contourf(X, Y, Z, 12)),
    ]


def _png_case_default(viewer):
    X, Y, Z = _grid_data()
    viewer.contourf(X, Y, Z, 12)
    viewer.title('Contourf Default')
...
@pytest.mark.parametrize('case_name,run_case', _contourf_cases(), ids=lambda value: value if isinstance(value, str) else None)
def test_contourf_variants(piscope_session, case_name, run_case):
    del case_name
...
def test_contourf_png_regression_cases(piscope_session, case_name, case_func):
